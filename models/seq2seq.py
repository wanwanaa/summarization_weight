...
class Seq2seq(nn.Module):
    def __init__(self, encoder, encoder_sent, decoder, config):
        super().__init__()
        self.encoder = encoder
        self.encoder_sent = encoder_sent
        self.decoder = decoder
        self.bos = config.bos
        self.s_len = config.tgt_len
    ...
    def forward(self, x, y, masks):
        ...
        sents = self.encoder_sent(encoder_out, masks)
        ...
        for i in range(self.s_len):
            # print(outs.size())
            _, out, h = self.decoder(y_c[:, i], h, encoder_out, sents, masks)
    ...
    def sample(self, x, masks):
        ...
        for i in range(self.s_len):
            ...
            _, out, h = self.decoder(out, h, encoder_out, sents, masks)
