...
def compute_loss(result, y, config):
    result = result.contiguous().view(-1, config.vocab_size)
...
if __name__ == '__main__':
    config = Config()
    vocab = Vocab(config)
    tokenizer = vocab.idx2word

    parser = argparse.ArgumentParser()
    parser.add_argument('--batch_size', '-b', type=int, default=64, help='batch size for train')
    parser.add_argument('--epoch', '-e', type=int, default=30, help='number of training epochs')
